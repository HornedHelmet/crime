...
public:
	...
	// This should be called when the object is not currently in use, such as the user has swapped to a different view.
	// The object should not be destroyed but it should reset to a state where nothing is selected but user input may be saved.
	// Must always be called if this object is being hidden.
	virtual void Clean();
